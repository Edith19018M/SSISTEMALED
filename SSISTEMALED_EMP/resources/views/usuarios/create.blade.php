...
<form action="/usuarios" method="POST">
    ...
    <div class="card card-outline card-danger">
        ...
        <div class="card-body">
            ...
            {{-- Emprendedor vinculado (solo visible cuando se selecciona rol Emprendedor) --}}
            <div class="row" id="seccion-emprendedor" style="display:none;">
                <div class="col-md-12">
                    <div class="form-group">
                        <label>Emprendedor Vinculado</label>
                        <select name="emprendedor_id" class="form-control @error('emprendedor_id') is-invalid @enderror">
                            <option value="">— Sin vincular —</option>
                            @foreach($emprendedores as $emp)
                                <option value="{{ $emp->id_emprendedor }}" {{ old('emprendedor_id') == $emp->id_emprendedor ? 'selected' : '' }}>
                        ...
                        @error('emprendedor_id')<span class="invalid-feedback">{{ $message }}</span>@enderror
                    </div>
                </div>
            </div>
        </div>
